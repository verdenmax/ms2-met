...
- `get_dia_data_object()`：新建 `DIAData`，注入 centroid 参数后 `_load_from_mzml(tot_raw_path)`。
...
- `DataManager` 当前并不缓存 `DIAData` 实例本身，每次 `get_dia_data_object` 都新建并重新读 mzML。
