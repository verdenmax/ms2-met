...
| `pred_integrate.*`（谱库 I1，Phase 2a） | 接入 feature_type=0 | `compute_speclib_i1`（按 ion-type 谱角）+ `compute_speclib_i2_i3_j2`（I2/I3/J2，Phase 2b）→ `single_pair_work` 出 `spec_pattern_*`/`pred_hl_ratio_*`/`pred_coverage*`/`unexpected_heavy_*`/`has_lib_pred`/窗口 meta |
