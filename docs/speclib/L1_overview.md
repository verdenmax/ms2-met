...
谱库的**预测碎片强度**已开始接入 SILAC 特征：`workflows/pred_features.py`（谱角/Spearman/top-K/I1 纯函数）、`workflows/pred_store.py`（肽段→预测一遍流式 lookup）与 `tools/speclib_sanity.py`（前置 go/no-go gate）构成 Phase 1 基础。**Phase 2a/2b 已接入主流程的 `feature_type=0` 路径**（`workflows/pred_integrate.py`：I1 强度模式 + I2 H/L 比一致性 + I3 预测覆盖度 + J2 意外峰污染 → `single_pair_work` 输出 `spec_pattern_*`/`pred_hl_ratio_*`/`pred_coverage*`/`unexpected_heavy_*` 等列，speclib 关闭则回退现状）。设计与后续接入（Phase 2c：J5 自适应判据 + `feature_type=1/2` + 提速）见 `docs/specs/2026-06-08-speclib-predicted-intensity-features-design.md`（v1.2，含 §11 实测验证），实现计划见 `docs/superpowers/plans/2026-06-08-speclib-pred-features-phase1.md`、`2026-06-09-speclib-pred-features-phase2a.md` 与 `2026-06-09-speclib-pred-features-phase2b.md`。
